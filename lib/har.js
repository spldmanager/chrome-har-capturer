...
function parseEntry(pageref, entry) {
    ...
    if(!entry.responseParams) entry.responseParams = {response:{}}
...
function parseHeaders(httpVersion, request, response) {
    if(!response.headers) return {request: {},response: {}}
...
function computeTimings(entry) {
    if(!entry.responseParams.response.timing) return {}
...
function computePayload(entry, headers) {
    if(!headers.response)  return {request:{},response: {}}
    // From Chrome:
